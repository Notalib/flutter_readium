...
- `bin/readium_versions` — print the currently pinned Readium toolkit versions from source-of-truth files.
...
- **Ask before committing to a solution with obvious downsides:** if a planned approach has significant trade-offs, platform gaps, or multiple reasonable alternatives, surface them to the user with a brief summary of options before implementing. Don't silently pick the path of least resistance — a short "here are the options" saves a revert later. Example: the decision to document the Android `allowedDefaultActions` limitation rather than reimplement system actions was made after asking the user, not unilaterally.
