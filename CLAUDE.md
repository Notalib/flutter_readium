...
- **Before declaring changes done, run only the gates relevant to the files changed:** Dart changes (`*.dart`, pubspecs, or analysis configuration) require `bin/format` + `bin/analyze`. For native or web changes, read and run the applicable platform gates in `flutter_readium/ios/CLAUDE.md`, `flutter_readium/android/CLAUDE.md`, or `flutter_readium/web/CLAUDE.md`. Cross-platform changes require the union of the applicable gates; documentation-only changes require no code gate.
