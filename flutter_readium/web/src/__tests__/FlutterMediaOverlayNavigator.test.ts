...
describe("buildAudioReadingOrder", () => {
  ...
  it("absolutizes a root-relative self link so the sub-path is not doubled", () => {
    ...
    // self link (e.g. "/test-fixtures/overlay/manifest.json") used to produce
    // non-absolute audio hrefs that the upstream AudioNavigator re-resolved against
    // the base, doubling the sub-path (".../overlay/overlay/chap1.mp3" → 404).
    ...
    try {
      const items = [makeItem("chap1.mp3", 0, 5)];
      const pub = fakePub("/test-fixtures/overlay/manifest.json");
      const ro = buildAudioReadingOrder(items, pub);
      expect(ro[0].href).toBe("http://localhost/test-fixtures/overlay/chap1.mp3");
    } finally {
