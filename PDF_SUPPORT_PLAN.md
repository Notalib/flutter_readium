...
- ✅ **Phase 9** — docs & changelog. Added `## Unreleased` section to both `flutter_readium/CHANGELOG.md` and `flutter_readium_platform_interface/CHANGELOG.md` with consolidated PDF feature entries. Updated `README.md` Features list to mention PDF reading, updated Supported Formats table (added PDF row), and added PDF reading / PDF preferences rows to the Platform Support table.
